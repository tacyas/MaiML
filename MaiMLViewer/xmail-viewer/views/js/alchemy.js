  ...
  Alchemy.prototype.interactions = function(instance) {
    ...
    return {
      edgeClick: function(d) {
        ...
        //MNTMNTMNT
        $('#txtSourceNode').val(edge._properties.source);
        $('#txtDestinationNode').val(edge._properties.target);
      ...
      nodeClick: function(n) {
		...
		$('#txtPlaceId').val(node._properties.pid);
		$('#txtXmailNid').val('[' + node._properties.xmail_nid + ']' +
        (node._properties.ownNode ? '' : (', now editing [' + getParam('id') +']'))
      );

		setNodeIdToText(node._properties.id);
...
// Set node id to source -> destination automatically.
function setNodeIdToText(node_id) {
	var currentSourceValue = $('#txtNewSourceNode').val();
	var currentDestinationValue = $('#txtNewDestinationNode').val();

	if (currentSourceValue == "") {
		$('#txtNewSourceNode').val(node_id);
	} else {
		if (currentDestinationValue == "") {
			$('#txtNewDestinationNode').val(node_id);
		} else {
			$('#txtNewSourceNode').val(node_id);
