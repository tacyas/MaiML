...
with open(codedir+'LOG/log_config.json', 'r') as f:
...
def readxl(path):
    if path == '':
        path = input_dir+'newmaimldata.xlsx'
...
def createcsv( resultId=[], selectkey=[], maiml_file_path = '', xl_file_path=output_dir+'output.xlsx'):
...
############################
def openinputjson():
    jsonfile = open(input_dir+'input.json', 'r')
    ...
    if 'maiml_file_name' not in inputlist or inputlist['maiml_file_name'] == '':
        ...
        inputlist['maiml_file_name'] = input_dir + inputlist['maiml_file_name']
    if 'xl_file_name' not in inputlist or inputlist['xl_file_name'] == "":
        ...
        inputlist['xl_file_name'] = output_dir + inputlist['xl_file_name']
...
def testmethod(args):
    input_filepath = ''
    if args.json :
        loggerI.info('use json file. Input json-filepath="{0}"'.format(input_dir+'input.json'))
    ...
    input_filepath = input_dir+'test.maiml'
    output_filepath = output_dir+'newmaimldata.xlsx'
    ###引数チェック
    ## OK
    input_filepath = input_dir+'test1.maiml'
    output_filepath = output_dir+'newmaimldata1.xlsx'
    ...
    ## OK
    input_filepath = input_dir+'test1.maiml'
    output_filepath = output_dir+'newmaimldata2.xlsx'
    ...
    input_filepath = input_dir+'test5.maiml'
    output_filepath= output_dir+'newmaimldata5.xlsx'
    createcsv(maiml_file_path=input_filepath, xl_file_path=output_filepath)
    # OK : 汎用データが存在しないファイル2:resultコンテンツがない-->空のエクセルファイルを出力
    input_filepath = input_dir+'test6.maiml'
    output_filepath = output_dir+'newmaimldata6.xlsx'
    createcsv(maiml_file_path=input_filepath, xl_file_path=output_filepath)
    # OK : 汎用データが存在しないファイル3:resultはあるが汎用データコンテンツがない-->空のエクセルファイルを出力
    input_filepath = input_dir+'test7.maiml'
    output_filepath = output_dir+'newmaimldata7.xlsx'
    createcsv(maiml_file_path=input_filepath, xl_file_path=output_filepath)
    # OK : 様々なテストデータ1
    input_filepath = input_dir+'test8.maiml'
    output_filepath= output_dir+'newmaimldata8.xlsx'
    createcsv(maiml_file_path=input_filepath, xl_file_path=output_filepath)
    # OK : 様々なテストデータ2
    input_filepath = input_dir+'test9.maiml'
    output_filepath = output_dir+'newmaimldata9.xlsx'
    createcsv(maiml_file_path=input_filepath, xl_file_path=output_filepath)
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        if args.json:  #if(len(sys.argv) <= 1):
            ...
            input_filepath = input_dir+args.maiml
            if args.xl == '':
                output_filepath = output_dir+'output.xlsx'
                loggerI.info('use default outout filepath : "{0}".'.format(output_filepath))
            else:
                output_filepath = output_dir+args.xl
            if args.selectid != '':
                resultID = args.selectid 
            if args.selectkey != '':
                selectkey = args.selectkey
