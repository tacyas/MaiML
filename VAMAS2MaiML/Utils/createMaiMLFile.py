...
class UpdateMaiML():
    ...
    def copytemplate(self, template_dict):
        instancelist = []
        instanceIDList = []
    ...
    ## method内のinstructionに関係付けられたtemplate要素のIDを検索しListを作成する
    ### method１つにつき、resultsを１つ（複数回実施は１つ以上）作成するため
    def createRelationDatafromInstID(self,protocoldict):
        # instruction-->transitionRef-->transitionID=arc/source,target-->arc/target,source=placeID-->placeRef-->template_id
        relationList = []
        '''
        relationList = [
            {$methodID:{'programIDlist':[],
                        'instructionIDlist':[],
                        'templateIDlist':[]}},
        ]
        '''
        ...
        ## 1. protocol層のtemplateを取得（全てのmethodに共通）
        templatelist = []
        ...
        ## 2. １つのmethodごとにplaceIDのリストを作成
        methodList = protocoldict[maimlelement.method]
        methodList = methodList if isinstance(methodList, list) else [methodList]
        for methoddict1 in methodList:
            programIDlist = []
            instructionIDlist = []
            templateIDlist = []
            
            templatelist1 = []
            templatelist1.append(templatelist)
            ## method層のtemplateを取得
            if maimlelement.materialTemplate in methoddict1:
                templatelist1.append(methoddict1[maimlelement.materialTemplate])
            if maimlelement.conditionTemplate in methoddict1:
                templatelist1.append(methoddict1[maimlelement.conditionTemplate])
            if maimlelement.resultTemplate in methoddict1:
                templatelist1.append(methoddict1[maimlelement.resultTemplate])
            
            # programからinstruction
            programlist = methoddict1[maimlelement.program] if isinstance(methoddict1[maimlelement.program],list) else [methoddict1[maimlelement.program]]
            pnmllist = methoddict1[maimlelement.pnml] if isinstance(methoddict1[maimlelement.pnml],list) else [methoddict1[maimlelement.pnml]]
            for programdict1 in programlist:
                programIDlist.append(programdict1[maimlelement.idd])
                ## program層のtemplateを取得
                if maimlelement.materialTemplate in programdict1:
                    templatelist1.append(programdict1[maimlelement.materialTemplate])
                if maimlelement.conditionTemplate in programdict1:
                    templatelist1.append(programdict1[maimlelement.conditionTemplate])
                if maimlelement.resultTemplate in programdict1:
                    templatelist1.append(programdict1[maimlelement.resultTemplate])
            
                ## instructionから関係するtemplateIDを取得
                instructionlist = programdict1[maimlelement.instruction] if isinstance(programdict1[maimlelement.instruction],list) else [programdict1[maimlelement.instruction]]
                for instructiondict1 in instructionlist:
                    instID = instructiondict1[maimlelement.idd]
                    instructionIDlist.append(instID)
                    transitionlist = instructiondict1[maimlelement.transitionRef] if isinstance(instructiondict1[maimlelement.transitionRef],list) else [instructiondict1[maimlelement.transitionRef]]
                    for transitiondict1 in transitionlist:
                        tranID = transitiondict1[maimlelement.refd]
                        for pnmldict1 in pnmllist:
                            arclist = pnmldict1[maimlelement.arc] if isinstance(pnmldict1[maimlelement.arc],list) else [pnmldict1[maimlelement.arc]]
                            for arcdict1 in arclist:
                                placeID__ = ''
                                if arcdict1[maimlelement.sourced]==tranID:
                                    placeID__ = arcdict1[maimlelement.targetd]
                                elif arcdict1[maimlelement.targetd]==tranID:
                                    placeID__ = arcdict1[maimlelement.sourced]
                                else:
                                    pass
                                for templatedict1 in templatelist1:
                                    placereflist = templatedict1[maimlelement.placeRef] 
                                    for placerefdict1 in placereflist:
                                        if placerefdict1[maimlelement.refd] == placeID__:
                                            templateIDlist.append(templatedict1[maimlelement.idd])
            relationList.append([{methoddict1[maimlelement.idd]:{'programIDlist':programIDlist,
    ...
    ## method内のinstructionに関係付けられたtemplate要素からインスタンスを作成する
    ### method１つにつき、resultsを１つ（複数回実施は１つ以上）作成
    def createResultslistfromProtocol(self,protocoldict, tmp_results_dict):
        # instruction-->transitionRef-->transitionID=arc/source,target-->arc/target,source=placeID-->placeRef-->template_id
        new_results_list = []
        material_list = []
        condition_list = []
        ...
        templatelist = {'materialTemplate':[],
                        ...
                        'resultTemplate':[]}
        if maimlelement.materialTemplate in protocoldict:
            mtlist = protocoldict[maimlelement.materialTemplate] if isinstance(protocoldict[maimlelement.materialTemplate],list) else [protocoldict[maimlelement.materialTemplate]]
            templatelist[maimlelement.materialTemplate].extend(mtlist)
        if maimlelement.conditionTemplate in protocoldict:
            ctlist = protocoldict[maimlelement.conditionTemplate] if isinstance(protocoldict[maimlelement.conditionTemplate],list) else [protocoldict[maimlelement.conditionTemplate]]
            templatelist[maimlelement.conditionTemplate].extend(ctlist)
        if maimlelement.resultTemplate in protocoldict:
            rtlist = protocoldict[maimlelement.resultTemplate] if isinstance(protocoldict[maimlelement.resultTemplate],list) else [protocoldict[maimlelement.resultTemplate]]
            templatelist[maimlelement.resultTemplate].extend(rtlist)
        
        ## 2. １つのmethodごとにplaceIDのリストを作成
        methodList = protocoldict[maimlelement.method]
        methodList = methodList if isinstance(methodList, list) else [methodList]
        for methoddict1 in methodList:
            new_results_dict = copy.deepcopy(tmp_results_dict)
            new_results_dict[maimlelement.uuid] = str(UUID.uuid4())
            new_results_dict[maimlelement.idd] = str(methoddict1[maimlelement.idd]) + '_results'

            programIDlist = []
            instructionIDlist = []
            
            templatelist1=templatelist
            '''
            templatelist1 = {'materialTemplate':[],
                             'conditionTemplate':[],
                             'resultTemplate':[]}
            '''
            ## method層のtemplateを取得
            if maimlelement.materialTemplate in methoddict1:
                mtlist = methoddict1[maimlelement.materialTemplate] if isinstance(methoddict1[maimlelement.materialTemplate],list) else [methoddict1[maimlelement.materialTemplate]]
                templatelist1[maimlelement.materialTemplate].extend(mtlist)
            if maimlelement.conditionTemplate in methoddict1:
                ctlist = methoddict1[maimlelement.conditionTemplate] if isinstance(methoddict1[maimlelement.conditionTemplate],list) else [methoddict1[maimlelement.conditionTemplate]]
                templatelist1[maimlelement.conditionTemplate].extend(ctlist)
            if maimlelement.resultTemplate in methoddict1:
                rtlist = methoddict1[maimlelement.resultTemplate] if isinstance(methoddict1[maimlelement.resultTemplate],list) else [methoddict1[maimlelement.resultTemplate]]
                templatelist[maimlelement.resultTemplate].extend(rtlist)
            ...
            for programdict1 in programlist:
                ...
                ## program層のtemplateを取得
                if maimlelement.materialTemplate in programdict1:
                    mtlist = programdict1[maimlelement.materialTemplate] if isinstance(programdict1[maimlelement.materialTemplate],list) else [programdict1[maimlelement.materialTemplate]]
                    templatelist1[maimlelement.materialTemplate].extend(mtlist)
                if maimlelement.conditionTemplate in programdict1:
                    mtlist = programdict1[maimlelement.conditionTemplate] if isinstance(programdict1[maimlelement.conditionTemplate],list) else [programdict1[maimlelement.conditionTemplate]]
                    templatelist1[maimlelement.conditionTemplate].extend(mtlist)
                if maimlelement.resultTemplate in programdict1:
                    mtlist = programdict1[maimlelement.resultTemplate] if isinstance(programdict1[maimlelement.resultTemplate],list) else [programdict1[maimlelement.resultTemplate]]
                    templatelist1[maimlelement.resultTemplate].extend(mtlist)
                        ...
                        for pnmldict1 in pnmllist:
                            ...
                            for arcdict1 in arclist:
                                ...
                                if arcdict1[maimlelement.sourced]==tranID:
                                    ...
                                    pass
                                #for templatedict1 in templatelist1:
                                for tempname, items in templatelist1.items(): # material/condition/result
                                    for templatedict1 in items:
                                        placereflist = templatedict1[maimlelement.placeRef] if isinstance(templatedict1[maimlelement.placeRef], list) else [templatedict1[maimlelement.placeRef]]
                                        for placerefdict1 in placereflist:
                                            if placerefdict1[maimlelement.refd] == placeID__:
                                                ## template-->instance
                                                updateMaiml = UpdateMaiML()
                                                instancedict1 = updateMaiml.copytemplate(templatedict1) ## list対応
                                                if tempname == maimlelement.materialTemplate:
                                                    material_list.append(instancedict1)
                                                elif tempname == maimlelement.conditionTemplate:
                                                    condition_list.append(instancedict1)
                                                elif tempname == maimlelement.resultTemplate:
                                                    result_list.append(instancedict1)
                                            
            new_results_dict[maimlelement.material]=material_list
            new_results_dict[maimlelement.condition]=condition_list
            new_results_dict[maimlelement.result]=result_list
    ...
    def createFullMaimlDict(self,maiml_dict):
        ...
        ###  --> methodごとにresultsを作成する
        ### templateの切り分けは難しいため、method内のinstructionに紐づくtemplateを全てresults下にインスタンスとして複製する
        updateMaiml = UpdateMaiML()
        tmp_results_dict = new_data_dict[maimlelement.results]
        new_results_list = ''
        try:
            new_results_list =  updateMaiml.createResultslistfromProtocol(protocol_dict, tmp_results_dict)
        except Exception as e:
            print("Error in createResultslistfromProtocol.")
            raise e
        '''
        ...
        for method_dict in method_list:
            ...
            for program_dict in program_list:
                ...
                # instructionからPNMLを検索し、紐づくtemplateを取得
                instprogIDdict__ = []
                instructionlist = program_dict[maimlelement.instruction] if isinstance(program_dict[maimlelement.instruction],list) else [program_dict[maimlelement.instruction]]
                for instructiondict in instructionlist:
                    instprogIDdict__.update({
                        instructiondict[maimlelement.idd]:{
                        'programID':program_dict[maimlelement.idd],
                        'methodID':method_dict[maimlelement.idd],
                    }})
                
            new_results_dict.update({maimlelement.material:material_list})
            new_results_dict.update({maimlelement.condition:condition_list})
            new_results_dict.update({maimlelement.result:result_list})
            new_results_list.extend([new_results_dict])
        ...
        ''' 
        new_data_dict[maimlelement.results]=new_results_list
